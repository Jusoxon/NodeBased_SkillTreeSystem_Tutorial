...
public class NodeBasedEditor : EditorWindow
{
    ...
    private void LoadNodes()
    {
        ...
        if (File.Exists(path))
        {
            ...
            if (File.Exists(path))
            {
                ...
                skillTrees = new Skill[skillData.skilltree.Length];
                skillTrees = skillData.skilltree;

                for (int i = 0; i < skillTrees.Length; ++i)
                {
                    for (int j = 0; j < loadedData.nodeDataCollection.Length; ++j)
                    {
                        if (loadedData.nodeDataCollection[j].id_Node == skillTrees[i].id_Skill)
                        {
                            pos = loadedData.nodeDataCollection[j].position;
                            break;
                        }
                    }
                    LoadSkillCreateNode(skillTrees[i], pos);
                    if (skillTrees[i].skill_Dependencies.Length == 0)
                    {
                        originNode.Add(skillTrees[i]);
                    }
                    skillDictionary.Add(skillTrees[i].id_Skill, skillTrees[i]);
                }
    ...
    private void LoadSkillCreateNode(Skill skill, Vector2 position)
    {
        ...
        nodes.Add(new Node(position, 200, 100, nodeStyle, selectedNodeStyle,
            inPointStyle, outPointStyle, OnClickInPoint, OnClickOutPoint, OnClickRemoveNode, 
            skill.id_Skill, skill.unlocked, skill.cost, skill.skill_Dependencies,skill.name,skill.explain));
        ++nodeCount;
    }
}